        ...
        <div class="card-body">
          <form action="{{ route('kwh-meter-request.update', $kwh_meter_request->id) }}" method="POST">
            ...
            <div class="row">
              ...
              <div class="col-lg-2">
                <div class="mb-2">
                  <label for="seriv_number" class="form-label mb-1">Seriv Number</label>
                    <select class="form-select" id="seriv_number" name="seriv_number" {{ Auth::user()->hasRole('Admin') || Auth::user()->hasRole('CETD (Dexter)') ? '' : 'disabled' }}>
                        <option value="">Select Seriv Number</option>
                        @foreach($serivs as $seriv)
                          <option value="{{ $seriv->seriv_number }}" @selected($kwh_meter_request->seriv_number == $seriv->seriv_number)>{{ $seriv->seriv_number }}</option>
                        @endforeach
                    </select>
                    <input type="hidden" name="is_assigning_of_seriv" value="yes">
                </div>
              </div>         
                    ...
                    <div class="col-lg-3 col-md-6">
                      ...
                      <div class="col-lg-3 col-md-12">
                          <div class="border rounded-3 p-3 h-100 bg-light">
                              ...
                              <h5 class="mb-2">
                                  <span class="badge fs-6 px-3 py-2 rounded-pill {{ $kwh_meter_request->is_liquidated && $kwh_meter_request->seriv_number <> null ? 'bg-success' : ($kwh_meter_request->is_liquidated && $kwh_meter_request->seriv_number == null ? 'bg-warning' : ($kwh_meter_request->approved_at ? 'bg-info' : ($kwh_meter_request->disapproved_at ? 'bg-danger' : 'bg-secondary'))) }}">
                                    {{ $kwh_meter_request->is_liquidated && $kwh_meter_request->seriv_number <> null ? 'Liquidated' : ($kwh_meter_request->is_liquidated && $kwh_meter_request->seriv_number == null ? 'Partially Liquidated' : ($kwh_meter_request->approved_at ? 'Approved' : ($kwh_meter_request->disapproved_at ? 'Disapproved' : 'Pending'))) }}
                                  </span>
                              </h5>
              ...
              <div class="col-lg-12">
                <div class="mb-2">
                  <label for="serial_numbers" class="form-label mb-1 fw-bold">Meter Serial Numbers <h1 class="badge bg-primary text-white">{{ $kwh_meter_request->quantity }}</h1></label>
            ...
            <div class="row">
              <div class="col-lg-12 text-end pt-2 gap-2">
                ...
                @if((Auth::user()->hasRole('Admin') || Auth::user()->hasRole('CETD (Dexter)')) && $kwh_meter_request->is_liquidated == true)
