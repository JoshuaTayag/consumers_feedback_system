      ...
      <div class="card">
        <div class="card-header">
          <div class="row align-items-center">
              <div class="col-lg-6">
                  <span class="mb-0 align-middle fs-3">Create Meter Type</span>
              </div>
          </div>
        </div>
        <div class="card-body">
          <form action="{{ route('meter-type.store') }}" method="POST">
            @csrf
            <div class="row">
              <div class="col-lg-2">
                <div class="mb-2">
                  <label for="meter_brand" class="form-label mb-1">Meter Brand</label>
                    <input type="text" class="form-control" id="meter_brand" name="meter_brand" old="meter_brand" required>
                </div>
              </div>
              <div class="col-lg-2">
                <div class="mb-2">
                  <label for="meter_code" class="form-label mb-1">Meter Code</label>
                    <input type="text" class="form-control" id="meter_code" name="meter_code" old="meter_code" required>
                </div>
              </div>
              <div class="col-lg-2">
                <div class="mb-2">
                  <label for="meter_type" class="form-label mb-1">Meter Type</label>
                    <select class="form-select" id="meter_type" name="meter_type" required>
                      ...
                      @foreach(config('constants.meter_control_type') as $controlType)
                          <option value="{{ $controlType['name'] }}">{{ $controlType['name'] }}</option>
                      @endforeach
                    </select>
                </div>
              </div>
              <div class="col-lg-6">
                <div class="mb-2">
                  <label for="meter_description" class="form-label mb-1">Meter Description</label>
                    <input type="text" class="form-control" id="meter_description" name="meter_description" old="meter_description" required>
                </div>
              </div>
